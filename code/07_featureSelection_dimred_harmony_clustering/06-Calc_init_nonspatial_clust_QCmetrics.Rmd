---
title: "06-Calc_init_nonspatial_clust_QCmetrics"
...
## get the autoannotation lookup tables for these clusterings so that we can have probable LC/raphe/gabaergic clusters labeled in outputs
annolut <- readRDS("processed-data/07_featureSelection_dimred_harmony_clustering/05b-init_clusterings_autoannoLCrapheAndGABAergic.RDS")
```
...
## run silhouette width approximation for each clustering result and the underlying (glm)pca mat
silres <- mclapply(X=featdrnames,mc.cores=5,FUN=function(fdrn){
    ...
    for (i in c(1:length(getcols))){
        ...
        # get the auto-annotations
        curlut <- copy(annolut[[grep(names(annolut),pattern=getcols[i],value=T)]])
        clusts <- merge.data.table(clusts,curlut,by.x=getcols[i],by.y="clusid")
        ...
        silwidths <- approxSilhouette(x = curdr,clusters = clusts$autoanno)
        subl[[i]] <- silwidths
...
### save results
saveRDS(silres2,"processed-data/07_featureSelection_dimred_harmony_clustering/06a-init_SNNk10_louvainClusts_silwidths.RDS")
...
clpur <- mclapply(X=featdrnames,mc.cores=5,FUN=function(fdrn){
    ...
    for (i in c(1:length(getcols))){
        ...
        clusts <- clusts[rownames(curdr),]
        clusts$rn <- NULL
        clustpurity <- neighborPurity(x = curdr,clusters = clusts$autoanno)
        subl[[i]] <- clustpurity
        names(subl)[i] <- getcols[i]
...
### save results
saveRDS(clpur2,"processed-data/07_featureSelection_dimred_harmony_clustering/06b-init_SNNk10_louvain_clustpuritys.RDS")
...
crmsd <- mclapply(X=featdrnames,mc.cores=5,FUN=function(fdrn){
    ...
    for (i in c(1:length(getcols))){
        ...
        cRMSD <- clusterRMSD(x = curdr,clusters = clusts$autoanno)
        subl[[i]] <- cRMSD
        names(subl)[i] <- getcols[i]
...
saveRDS(crmsd2,"processed-data/07_featureSelection_dimred_harmony_clustering/06c-init_SNNk10_louvain_clustRMSDs.RDS")
```
