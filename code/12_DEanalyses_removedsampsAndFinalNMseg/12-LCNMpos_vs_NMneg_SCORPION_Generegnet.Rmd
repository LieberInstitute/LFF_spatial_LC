...
## but then   no applicable method for `@` applied to an object of class "matrix" coming from somewehere ca line 110 in runPANDA -_- 
## add line 150 to runPANDA: geneCoexp <- as(geneCoexp,"dgCMatrix")

scorpii <- lapply(xprmats,FUN=function(x){
  scorpi <- scorpion(tfMotifs = tftab,
                   gexMatrix = x,
                   ppiNet = hsaPPI,
                   nCores = 8,
                   gammaValue = 6,
                   ...
                   showProgress = TRUE,
                   randomizationMethod = "within.gene",
                   filterExpr = TRUE,scaleByPresent = FALSE)
  return(scorpi)
})

saveRDS(scorpii,"processed-data/12_DEanalyses_removedsampsAndFinalNMseg/12-LCNMpos_NMneg_scorpii.RDS")
```
── SCORPION ───────────────────────────────────────────────────────────────────────────────────
