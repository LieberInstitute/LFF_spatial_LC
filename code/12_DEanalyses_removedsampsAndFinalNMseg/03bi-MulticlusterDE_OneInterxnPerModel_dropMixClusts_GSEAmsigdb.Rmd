...
```{r}
de2b <- readRDS("processed-data/12_DEanalyses_removedsampsAndFinalNMseg/02b-voomLmFit_objs_and_DE_tables_LabSex_LabE2vE4_LabSingleGenos_LabYRI_noMixClusts.RDS")
...
### YRI tests are fucked up don't use them
de2b <- de2b2[grep(names(de2b2),pattern="YRI",value=T,invert=T)]
rm(de2b2)

### drop sex chr genes from sex DE tests
de2b[grep(names(de2b),pattern="sex",value=T)] <- lapply(de2b[grep(names(de2b),pattern="sex",value=T)],FUN=function(x){
    x[!(chr %in% grep(chr,pattern="chrX|chrY",value=T))]})

### drop mito genes from all tests
de2b <- lapply(de2b,FUN=function(x){
...
i<-1
for (i in c(1:length(de2b))){
    setnames(de2b[[i]],"t","tstat")
    tmp <- copy(de2b[[i]])[,.(gene_id,tstat)]
    ...
    names(deres)[length(deres)] <- names(de2b)[i]
    rm(v,tmp)
}
rm(i,de2b)
```
...
saveRDS(gseares,"processed-data/12_DEanalyses_removedsampsAndFinalNMseg/03bi-MulticlusterDE_OneInterxnPerModel_dropMixClusts_GSEAmsigdb.RDS")
...
for (i in c(1:length(de2b))){
    setnames(de2b[[i]],"t","tstat")
    tmp <- copy(de2b[[i]])[,.(gene_id,abs(tstat))]
...
saveRDS(gseares.unsign,"processed-data/12_DEanalyses_removedsampsAndFinalNMseg/03bi-MulticlusterDE_OneInterxnPerModel_dropMixClusts_GSEAmsigdb_UNsigned.RDS")
```
