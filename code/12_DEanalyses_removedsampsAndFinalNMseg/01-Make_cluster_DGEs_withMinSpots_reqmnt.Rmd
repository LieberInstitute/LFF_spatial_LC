...
### now the important part: to model all data jointly, we need to make a cluster-sex-genotype pseudobulk object. then to run sex DE for one cluster we would do e.g. (LC_E2hetMale + LC_E2homMale + LC_E4hetMale + LC_E4homMale) - (LC_E2hetFem + LC_E2homFem + LC_E4hetFem + LC_E4homFem).
```{r}
...
dgels <- mapply(a=ag,n=names(ag),SIMPLIFY=FALSE,FUN=function(a,n){
    ...
    if(length(unique(s$Sex))<2){return()} else 
    {
        ...
        h <- hist(rowmn.m[which(rowmn.m>min(rowmn.m)&rowmn.m<(1+min(rowmn.m)))],breaks=10,plot=FALSE)
...
## save
```{r}
saveRDS(dgels,"processed-data/12_DEanalyses_removedsampsAndFinalNMseg/01a-DGElist_per_25HDG75SVGlouv1_cluster_normFilt_10spotmin.RDS")
```
...
clusterMap(d=dgels,n=names(dgels),SIMPLIFY=FALSE,cl=clus,fun=function(d,n){
  ...
  lapply(covcols,FUN=function(cov){
    ...
    png(paste0("plots/12_DEanalyses_removedsampsAndFinalNMseg/01_pseudobulk_cluster_MDSes/01a-",n,"_",cov,"_1_2_10spotfilt.png"),width=1000,height=800)
    ...
    png(paste0("plots/12_DEanalyses_removedsampsAndFinalNMseg/01_pseudobulk_cluster_MDSes/01a-",n,"_",cov,"_1_3_10spotfilt.png"),width=1000,height=800)
    plotMDS(d,col=pal[factor(d$samples[,cov])], dim.plot = c(1,3),pch = 18)
    legend('bottomright', legend=levels(d$samples[,cov]), col=pal, cex=1, pch=18)
    dev.off()
    
    png(paste0("plots/12_DEanalyses_removedsampsAndFinalNMseg/01_pseudobulk_cluster_MDSes/01a-",n,"_",cov,"_2_3_10spotfilt.png"),width=1000,height=800)
