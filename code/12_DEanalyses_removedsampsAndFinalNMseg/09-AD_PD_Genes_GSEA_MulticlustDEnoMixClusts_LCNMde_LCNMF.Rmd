...
de2b2[grep(names(de2b2),pattern="Sex",value=T)] <- lapply(de2b2[grep(names(de2b2),pattern="Sex",value=T)],FUN=function(x){
    y <- x[!(chr %in% grep(chr,pattern="chrX|chrY",value=T))]
    return(y)})

### drop mito genes from all tests
de2b2 <- lapply(de2b2,FUN=function(x){
    y <- x[!(chr %in% grep(chr,pattern="chrMT",value=T))]
    return(y)})
...
for (i in c(1:length(de2b2))){
    ...
    tmp <- copy(de2b2[[i]])[,.(gene_name,tstat)]
...
i<-1
for (i in c(1:length(de2b2))){
    tmp <- copy(de2b2[[i]])[,.(gene_name,abs(tstat))]
...
[13] parallelly_1.43.0           default_1.0.0              
[15] colorout_1.3-0.2            fgsea_1.32.4               
...
[55] ggtree_3.14.0           ggfun_0.1.8             generics_0.1.3         
