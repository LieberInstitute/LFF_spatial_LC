...
### we want to keep genes based on cluster.
...
## save
```{r}
saveRDS(dgel,"processed-data/12_DEanalyses_removedsampsAndFinalNMseg/01a-DGElist_allClust_10spotmin_edgeRdefaultLoExpFilt.RDS")
```
...
lapply(covcols,FUN=function(cov){
  ...
  png(paste0("plots/12_DEanalyses_removedsampsAndFinalNMseg/01_pseudobulk_cluster_MDSes/01a-Allclust-",cov,"_1_2_10spotfilt.png"),width=1000,height=800)
  plotMDS(dgel,col=pal[factor(dgel$samples[,cov])], dim.plot = c(1,2),pch = 18)
  legend('bottomright', legend=levels(dgel$samples[,cov]), col=pal, cex=1, pch=18)
  dev.off()
  
  png(paste0("plots/12_DEanalyses_removedsampsAndFinalNMseg/01_pseudobulk_cluster_MDSes/01a-Allclust-",cov,"_1_3_10spotfilt.png"),width=1000,height=800)
  plotMDS(dgel,col=pal[factor(dgel$samples[,cov])], dim.plot = c(1,3),pch = 18)
  legend('bottomright', legend=levels(dgel$samples[,cov]), col=pal, cex=1, pch=18)
  dev.off()
  
  png(paste0("plots/12_DEanalyses_removedsampsAndFinalNMseg/01_pseudobulk_cluster_MDSes/01a-Allclust-",cov,"_2_3_10spotfilt.png"),width=1000,height=800)
  plotMDS(dgel,col=pal[factor(dgel$samples[,cov])], dim.plot = c(2,3),pch = 18)
  legend('bottomright', legend=levels(dgel$samples[,cov]), col=pal, cex=1, pch=18)
  dev.off()
})
...
lapply(covcols,FUN=function(cov){
  ...
  ## 1,2 ##
  res <- plotMDS(dgel,dgel$samples[,cov], dim.plot = c(1,2),pch=18)
  dev.off()
  resdat <- as.data.table(cbind(res$x,res$y,rownames(dgel$samples),dgel$samples[,cov]))
  ...
  ## 1,3 ##
  res <- plotMDS(dgel,dgel$samples[,cov], dim.plot = c(1,3),pch=18)
  dev.off()
  resdat <- as.data.table(cbind(res$x,res$y,rownames(dgel$samples),dgel$samples[,cov]))
  ...
  png(paste0("plots/12_DEanalyses_removedsampsAndFinalNMseg/01_pseudobulk_cluster_MDSes/01a-Allclust_",cov,"_1_3_10spotfilt.png"),width=1000,height=800)
  print(p)
  dev.off()

    ## 2,3 ##
  res <- plotMDS(dgel,dgel$samples[,cov], dim.plot = c(2,3),pch=18)
  dev.off()
  resdat <- as.data.table(cbind(res$x,res$y,rownames(dgel$samples),dgel$samples[,cov]))
