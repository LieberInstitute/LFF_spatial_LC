...
### now we need to extract the counts matrix and logcounts matrix, match the old unique ids to the new ones in tmpcd$newrn, swap the rownames of the new coldata (tmpcd) to tmpcd$newrn, double-check they're in the same order, and then pass to SPE the following for the single-section SPE:
the new images slot
...
stopifnot(all(colnames(newlct)==rownames(tmpcd)))
stopifnot(all(colnames(newlct)==rownames(newspcoord)))
stopifnot(all(rownames(newlct)==rownames(rowData(lc2))))
...
lc2split <- SpatialExperiment(
    assays=list(counts=newct,logcounts=newlct),
    rowData=rowData(lc2),
    colData=tmpcd2,
...
## append chromosome information to rowData
```{r}
tmprd <- as.data.table(rowData(lc2split),keep.rownames=T)

enslut <- fread("raw-data/USEFOR10X_ensg_lut_hg38_fromEns98_111123.txt")

enslut <- unique(enslut[,.(ensembl_gene_id,chromosome_name)])
setnames(enslut,c("ens","chr"))

enslut <- enslut[ens %in% rownames(lc2split)]
enslut[chr %in% paste0(c(1:22,"X","Y","MT")),chr:=paste0("chr",chr)]

tmprd <- merge.data.table(tmprd,enslut,by.x="gene_id",by.y="ens",all.x=TRUE)
tmprd <- DataFrame(tmprd,row.names=tmprd$rn)[rownames(lc2split),]

rowData(lc2split) <- tmprd
```
...
### 01/05/25: also handy to make and save a second, 'lite' copy of the SPE that doesn't include image data or logcounts (which are trivial to recalculate on the fly, but take up a ton of space and read/write time in saved objects).
```{r}
lc2s.lite <- lc2s
rm(lcs2)

imgData(lc2s.lite) <- NULL
logcounts(lc2s.lite) <- NULL

saveRDS(lc2s.lite,"processed-data/06-countsOnly_noImgData_QCed_SPE_split_to_tissSections.RDS")
```
