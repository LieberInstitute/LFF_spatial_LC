...
## THIS SCRIPT HAS NOT YET BEEN RUN -- V13B23-366_A1 segmentations still pending ##

get NM segmentations systematically
```{r}
...
merge the NM segmentations into colDatas
```{r}
cdtxt2 <- merge.data.table(cdtxt,nmseglist,by.x="barcode",by.y="original_spot_id")
stopifnot(nrow(cdtxt2)==nrow(cdtxt))
fwrite(cdtxt,"processed-data/06-QCed_SPE_splitToTissSect_colData.txt",sep='\t',quote=F,row.names=F,col.names=T)

##
lcfullcd <- as.data.table(colData(lcfull),keep.rownames=T)
lcfullcd <- merge.data.table(lcfullcd,nmseglist,by.x="original_spot_id",by.y="barcode")
stopifnot(nrow(lcfullcd)==nrow(colData(lcfull)))
lcfullcd <- DataFrame(lcfullcd,row.names=lcfullcd$rn)[colnames(lcfull),]
lcfullcd$rn <- NULL
colData(lcfull) <- lcfullcd
saveRDS(lcfull,"processed-data/06-QCed_SPE_split_to_tissSections.RDS")

##
lclitecd <- as.data.table(colData(lclite),keep.rownames=T)
lclitecd <- merge.data.table(lclitecd,nmseglist,by.x="original_spot_id",by.y="barcode")
stopifnot(nrow(lclitecd)==nrow(colData(lclite)))
lclitecd <- DataFrame(lclitecd,row.names=lclitecd$rn)[colnames(lclitecd),]
lclitecd$rn <- NULL
colData(lclite) <- lclitecd
