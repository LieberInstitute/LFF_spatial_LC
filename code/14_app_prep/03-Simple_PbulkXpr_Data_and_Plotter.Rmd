---
title: "03-Simple_PbulkXpr_Data_and_Plotter"
...
pbulkdt <- readRDS("processed-data/14_apps_prep/02b-iSEE_pbulkobj_FOR_SHARING.RDS")
dat <- as.data.table(assays(pbulkdt)[2],keep.rownames=T)
dat[,group:=NULL]
dat[,group_name:=NULL]
setnames(dat,"rn","gene_name")

### row per gene per pseudobulk sample
dt <- melt.data.table(dat,id.vars="gene_name")
dt[,variable:=as.character(variable)]
tmpcd <- as.data.table(colData(pbulkdt),keep.rownames=T)

### append metadata
dt <- merge.data.table(dt,tmpcd,by.x="variable",by.y="rn")
setnames(dt,11,"Est.Pct.AA.Genomic.Ancestry")


setnames(dt,"value","expression")

fwrite(dt,"processed-data/14_apps_prep/03-Simple_pbulk_expression_viewerdata.txt.gz")
```
...
## load data.table object
dt <- fread("processed-data/14_apps_prep/03-Simple_pbulk_expression_viewerdata.txt.gz")
