...
LC2 = mean(BG_mask(:));
 ...
 %% Append to results table
    T = table({fname}, tis1, BB1, LC1, tis2,  BB2,  LC2, ...
              'VariableNames', {'fname', 'tis1', 'BB1', 'LC1', 'tis2', 'BB2', 'LC2'});
    results = [results; T];
 ...
 imshow(BGmask_tis1, []);
 title('BGmask\_tis1');
 xlabel(sprintf('Mean = %.4f', tis1));

 subplot(2,3,2);
 imshow(BGmask_BB1, []);
 title('BGmask\_BB1');
 xlabel(sprintf('Mean = %.4f', BB1));

 subplot(2,3,3);
 imshow(BGmask_LC1, []);
 title('BGmask\_LC1');
 xlabel(sprintf('Mean = %.4f', LC1));

 subplot(2,3,4);
 imshow(BGmask_tis2, []);
 title('BGmask\_tis2');
 xlabel(sprintf('Mean = %.4f', tis2));

 subplot(2,3,5);
 imshow(BGmask_BB2, []);
 title('BGmask\_BB2');
 xlabel(sprintf('Mean = %.4f', BB2));

 subplot(2,3,6);
 imshow(BGmask_LC2, []);
 title('BGmask\_LC2');
 xlabel(sprintf('Mean = %.4f', LC2));
