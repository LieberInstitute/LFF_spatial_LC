---
title: "04_02-LCsampsonly_to_fullData_registration"
...
for versioning consistency, rerun regist wrapper on the full SPE and its labels
```{r}
lc3 <- readRDS("processed-data/06-countsOnly_noImgData_QCed_SPE_split_to_tissSections.RDS")

labs <- readRDS("processed-data/07_featureSelection_dimred_harmony_clustering/04-Initial_louvLeid_res0.5-1-2_clusterings.RDS")
labs <- labs$HARMONYlmbna_HDG_SVG_2575[,.(rn,snnHARMONYlmbna_HDG_SVG_2575_louv_res1)]
setnames(labs,2,"clusid")

anno <- fread("processed-data/08_validitycheck_25hdg75svg_louv1/10-25hdg75svg_louv1_annots.txt")
labs <- merge.data.table(labs,anno[,.(clusid,anno)],by="clusid")
labs <- DataFrame(labs,row.names=labs$rn)[colnames(lc3),]
...
stopifnot(sum(flares$ID %in% unique(tmpcd$brnum))==length(unique(lc3$brnum)))
...
```{r}
fulldat.regw <- registration_wrapper(lc3,var_registration="anno",var_sample_id = "sample_id",covars=c("Sex","APOE","Age","YRI"),gene_ensembl = "gene_id",gene_name="gene_name",min_ncells=10)
```

reg wrapper results from sensitivity-analyzed lc3:
```{r}
lcsens.regw <- readRDS("processed-data/16_sensitivityanalysis_reproc_wo3donors/Sens2_markers_and_registration_final/01_2-LCsampsonly_HDGSVG2575louv1_regWrapOut.RDS")
```
...
```{r}
### for lc3:
lc3.mk <- as.data.table(fulldat.regw$enrichment)
...
lcsens.mk <- as.data.table(lcsens.regw$enrichment)
keepcols <- c("ensembl",grep(names(lcsens.mk),pattern="^t_stat_",value=T))
lcsens.mk <- lcsens.mk[,..keepcols]
    
setnames(lcsens.mk,c(2:ncol(lcsens.mk)),gsub(names(lcsens.mk)[c(2:ncol(lcsens.mk))],pattern="^t_stat_",replacement=""))

rm(lcsens.regw,fulldat.regw)
```

## now the rewritten registration function
```{r}
m <- copy(lc3.mk) # ref
s <- copy(lcsens.mk) # query
...
for (i in c(1:(length(j)+1))){
    ...
    for (k in c(2:nclhyp)){
        ...
        for (l in c(2:nclaba)){
            ...
            curhyp <- colnames(m)[k]
            curaba <- colnames(s)[l]
            setorderv(cortab,curhyp,-1) # by refs top n genes
    ...
    outlist[[i]] <- plttab
    names(outlist)[i] <- paste0("orig_to_sens_top_",ngene)
    rm(curj,plttab,cortab)
}

### append cluster annots
sensanno <- fread("processed-data/16_sensitivityanalysis_reproc_wo3donors/Sens1_LCsampsonly_featureSelection_dimred_harmony_clustering/08_2-25hdg75svg_louv1_annots.txt")

outlist <- lapply(outlist,function(y){
    y <- setnames(y,sensanno$clusid,sensanno$plot_anno,skip_absent = T)
    y <- merge.data.table(y,anno[,.(anno,plot_anno)],by.x="LFF_clus",by.y="anno")
    reordnames <- c("plot_anno",names(y)[!(names(y) %in% c("anno","LFF_clus"))])
    y <- y[,..reordnames]
    setnames(y,"anno","Full Data Cluster",skip_absent=T)
    return(y)
  })
  return(z)
})
...
```{r}
saveRDS(outlist,"processed-data/16_sensitivityanalysis_reproc_wo3donors/Sens2_markers_and_registration_final/04_2-fulldata_to_sensitivitydata_label_regists.RDS")
```
