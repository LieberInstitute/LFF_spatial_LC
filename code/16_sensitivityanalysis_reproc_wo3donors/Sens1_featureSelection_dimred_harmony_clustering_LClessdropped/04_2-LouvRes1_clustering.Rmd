...
```{r}
lcdimrs <- readRDS("processed-data/16_sensitivityanalysis_reproc_wo3donors/Sens1_LCsampsonly_featureSelection_dimred_harmony_clustering/02_02-PCA_harmo_UMAP_matrices_HDG_SVG_2575only.RDS")

## we want the reduction with harmony correction in the third slot.
...
# parallelize the clustering calls, which are cpu heavy tasks; data.table threads to 1 to prevent resource overcommit
setDTthreads(1,restore_after_fork = FALSE)

dr=clustdims
drn="HARMONYlmbna_HDG_SVG_2575"
...
louvnames <- paste0(paste0("snn",drn,"_louv_res"),1)
setnames(rettab,c("rn",louvnames))

fwrite(rettab,"processed-data/07_2_LCsampsonly_featureSelection_dimred_harmony_clustering/04_2-LConly_HDG_SVG_2575_Louv1_clustering.txt",sep='\t',quote=F,row.names=F)
