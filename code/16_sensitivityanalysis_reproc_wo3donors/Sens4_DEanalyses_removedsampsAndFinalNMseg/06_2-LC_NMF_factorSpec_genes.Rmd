---
title: "06_2-LC_NMF_factorSpec_genes"
...
lc.nmf2 <- readRDS("processed-data/16_sensitivityanalysis_reproc_wo3donors/Sens3_subclustering_and_NMF/07_2-lcnmf120_tol1e-9.RDS")
w2 <- as.data.table(lc.nmf2$w,keep.rownames=T)
w2 <- merge.data.table(w2,rddt[,.(gene_id,gene_name)],by.x="rn",by.y="gene_id")
nameord <- c("rn","gene_name",paste0("LCnmf",c(1:120)))
w2 <- w2[,..nameord]
...
fwrite(w2m.select,"processed-data/16_sensitivityanalysis_reproc_wo3donors/Sens3_subclustering_and_NMF/06_2-LCNMF_factors_and_factorSpecGenes_min25specGenes.txt",sep='\t',quote=F,row.names=F,col.names=T)
```
