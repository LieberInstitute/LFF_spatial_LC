...
imperfectsearch <- list.files("processed-data/05_QC/06b-Uncleaned_capture_area_sectionsplitting_spatialLIBD_outputs",full.names = TRUE)
...
```{r}
checkanno <- list.files("processed-data/05_QC/06b-Uncleaned_capture_area_sectionsplitting_spatialLIBD_outputs",full.names = TRUE)
...
```{r}
simplesplits <- fread("/Users/bmulvey/Desktop/KM Lab/local_lffLC/LFF_spatial_LC/processed-data/05_QC/06a-capture_areas_split_withOUT_slibd.txt")
...
fwrite(allfinalsectionsplits,"processed-data/05_QC/06c-raw_SPE_spots_assigned_to_tissue-section-1or2_for_all_capture_areas.txt",sep='\t',quote=F)
...
fwrite(fullres_with_splitannots,"processed-data/05_QC/06c-tissueOverlapping_spots_with_fullres-pixel-row-col-coords_and_section_annotations.txt",sep='\t',quote=F)
```
