...
## re-run on 02/18 to propagate NM segmentation information from coldata into DGElists and make an additional set of DGElists by NM segmentation-based subclusters.
...
pdf("plots/11_DEanalyses/01-logCPM_cutoff_checks.pdf",height=10,width=10)
...
## save
```{r}
saveRDS(dgels,"processed-data/11_DEanalyses/01a-DGElist_per_25HDG75SVGlouv1_cluster_normFilt.RDS")
```
...
clusterMap(d=dgels,n=names(dgels),SIMPLIFY=FALSE,cl=clus,fun=function(d,n){
  ...
  lapply(covcols,FUN=function(cov){
    ...
    png(paste0("plots/11_DEanalyses/01_pseudobulk_cluster_MDSes/01a-",n,"_",cov,"_1_2.png"),width=1000,height=800)
    ...
    png(paste0("plots/11_DEanalyses/01_pseudobulk_cluster_MDSes/01a-",n,"_",cov,"_1_3.png"),width=1000,height=800)
    plotMDS(d,col=pal[factor(d$samples[,cov])], dim.plot = c(1,3),pch = 18)
    legend('bottomright', legend=levels(d$samples[,cov]), col=pal, cex=1, pch=18)
    dev.off()
    
    png(paste0("plots/11_DEanalyses/01_pseudobulk_cluster_MDSes/01a-",n,"_",cov,"_2_3.png"),width=1000,height=800)
...
#### NM-SEGMENTATION-SUBCLUSTER DGELists ####
...
## # make pseudobulk objectS (lcadjacency and adjacency-by-NM) (dreamlet is v. fast at this)
```{r}
LCadj.ag <- dreamlet::aggregateToPseudoBulk(lc3[,lc3$LCadjacent!=""],sample_id="sample_id",cluster_id = "LCadjacent",BPPARAM = MulticoreParam(6))
...
# extract colData to feed into DGEList as the "samples" data
adj.agcd <- as.data.frame(colData(LCadj.ag))
nm.agcd <- as.data.frame(colData(LCNMadj.ag))

## only retain clusters for which at least 43 samples (of 85 total, i.e over half) are available after pseudobulking+filtering through dreamlet:  
LCadj.ag <- as.list(assays(LCadj.ag))
LCadj.agc <- lapply(LCadj.ag,FUN=function(a){ncol(a[,colSums(a)>0])})
LCadj.ag <- LCadj.ag[!(LCadj.agc<43)]

LCNMadj.ag <- as.list(assays(LCNMadj.ag))
LCNMadj.agc <- lapply(LCNMadj.ag,FUN=function(a){ncol(a[,colSums(a)>0])})
LCNMadj.ag <- LCNMadj.ag[!(LCNMadj.agc<43)]

## drop clusters where there are 20+ samples, but under 18 samples for one of the sexes (i.e., < half of the samples from the smaller group--females w 35 sections total):
agscs <- mapply(list(LCadj.ag,LCNMadj.ag),list(adj.agcd,nm.agcd),SIMPLIFY=FALSE,FUN=function(ag,agcd){
    agsc <- lapply(ag,FUN=function(a){
        cn <- colnames(a[,colSums(a)>0])
        tmp <- as.data.table(agcd[cn,])
        if(nrow(tmp[,.N,by="Sex"][N<18])>0){
...
LCadj.ag <- LCadj.ag[agscs[[1]]==1]
LCNMadj.ag <- LCNMadj.ag[agscs[[2]]==1]

rm(agscs,LCadj.agc,LCNMadj.agc)

# stratify by APOE4 positive / negative  
adj.agcd$e4 <- ifelse(adj.agcd$APOE %in% c("E4/ E4","E3/ E4"),yes="carrier",no="negative")
...
agcds <- list(adj.agcd,nm.agcd)
names(agcds) <- c("LCadj","LCNMadj")

sub.dgels <- mapply(list(LCadj.ag,LCNMadj.ag),names(agcds),SIMPLIFY=FALSE,FUN=function(ag,agcd.n){
  
  # get the appropriate metadata sheet
  if(agcd.n=="LCadj"){
    agcd <- copy(agcds$LCadj)
  } else {
    agcd <- copy(agcds$LCNMadj)
  }
  
  retdgels <- mapply(a=ag,n=names(ag),SIMPLIFY=FALSE,FUN=function(a,n){
    X <- a[,colSums(a)>0]
    s <- copy(agcd)[colnames(X),]
    if(length(unique(s$Sex))<2){return()} else {
...
names(sub.dgels) <- c("LCadj_subclusts","LCNMadj_subclusts")
```

## get and arrange low-expression-cutoff plots
```{r}
mapply(sub.dgels,names(sub.dgels),SIMPLIFY=FALSE,FUN=function(x,n){
  plts <- lapply(x,FUN=function(y){
    y$plt
  })
  pdf(paste0("plots/11_DEanalyses/01b-logCPM_cutoff_checks_",n,".pdf"),height=3.33*ceiling(length(plts)/3),width=10)
  do.call("grid.arrange",c(plts,ncol=3))
  dev.off()
})
```
...
```{r}
LCadj.dgels <- lapply(sub.dgels$LCadj_subclusts,function(x){x[[1]]})
nmadj.dgels <- lapply(sub.dgels$LCNMadj_subclusts,function(x){x[[1]]})

## save separate RDSes for each subclustering
saveRDS(LCadj.dgels,"processed-data/11_DEanalyses/01b-DGElist_per_LCadj_subcluster_normFilt.RDS")
saveRDS(nmadj.dgels,"processed-data/11_DEanalyses/01b-DGElist_per_LCNMadj_subcluster_normFilt.RDS")
