...
append ±1MB apoe haplotype-level local ancestry estimates; to get a single locus measure (since we don't have phased seq data), set to 0 for E4 negative donors (interaction effect automatically is then 0), the E4 haplotype local ancestry for hets, and sum of local ancestries for homozygotes.
...
la[hap1gt=="E4"&hap2gt!="E4",E4LA:=apoe_hap1_estPropYRI]
la[hap1gt!="E4"&hap2gt=="E4",E4LA:=apoe_hap2_estPropYRI]
la[hap1gt=="E4"&hap2gt=="E4",E4LA:=apoe_hap1_estPropYRI+apoe_hap2_estPropYRI]
...
dgels <- lapply(dgels,FUN=function(d){
    ...
    tmpsamp$rn <- NULL
    d$samples <- tmpsamp
    #use YRI for global ancestry x2 for consistency w/ the local ancestry estimates and ranges
    d$samples$YRI <- d$samples$YRI*2
    return(d)
})
```
...
fulldgelres.norin <- parallel::clusterApply(x=dgels,cl = clus,fun=function(X){
    ...
    dge <- X
    des <- model.matrix(~0+e4copies*E4LA+Sex+YRI+Age, data = dge$samples)
...
fulldgelres.rin <- parallel::clusterApply(x=dgels,cl = clus,fun=function(X){
    ...
    dge <- X
    des <- model.matrix(~0+e4copies*E4LA+Sex+YRI+Age+Rin, data = dge$samples)
    colnames(des)[colnames(des)=="e4copies:E4LA"] <- "e4copies_by_LA"
