---
title: "10aV2-speLCnmfPrefilt_projectToLuskinLCsubclusts"
...
```{r}
lcnmf <- readRDS("processed-data/10_subclustering/07thru10_ver2_53factorNMF/07b-lcnmf53_tol1e-9_rowSumLogctFilt18.RDS")
```
...
colnames(colData(mslc))[c(11:ncol(colData(mslc)))] <- paste0("LCspeFiltNMF_",c(1:53))
...
## 2nd/98% of scaled avgs are -1.5 and 0.89, but there's a couple extremes skewing the color scale so put tighter bounds on that for comprehensibility.

## for practical purposes, therefore, CHANGE THE SCALED AVG values to be just slightly above/below these
pltdat[lessOrEqual(scaled.avg,-1.5),scaled.avg:=-1.6]
pltdat[greaterOrEqual(scaled.avg,0.89),scaled.avg:=1]
pltdat <- pltdat[!is.na(prop)]

png("plots/10_subclustering/07thru10_ver2_53factorNMF/10a-speLCnmfPrefilt_allfactors_projectToLuskinLCsubclusts.png",height=1000,width=8000)
ggplot(pltdat, aes(x=nmf, y=msclust, size=prop, color=scaled.avg))+
  geom_count()+
  theme_bw()+
  scale_size(range=c(0,3))+
  scale_color_gradient2(low = "darkblue",mid="white",high="red",midpoint = 0,limits=c(-1.61,1.01),na.value = NA)+
...
factspec <- fread("processed-data/10_subclustering/07thru10_ver2_53factorNMF/09bV2-LCNMFLogCt18Filt_factors_and_factorSpecGenes_min25specGenes.txt")
gsfacts <- unique(factspec$factor)
gsfacts <- gsub(gsfacts,pattern="LCnmf(.*)$",replacement="\\1")
gsfacts <- paste0("LCspeFiltNMF_",gsfacts)

png("plots/10_subclustering/07thru10_ver2_53factorNMF/10a-speLCnmfPrefilt_GSEAdFactors_projectToLuskinLCsubclusts.png",height=1000,width=1500)
...
## well basically all of them have at least a non-negative projection value for their three believable NE clusters (NE_1/2/3) and are represented in virtually all cells of their LC subclusters. good sign!
```
