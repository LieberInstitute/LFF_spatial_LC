---
title: "10c-speLCnmfPrefilt_projectToLuskinFullDataset"
...
colnames(colData(lca))[c(14:ncol(colData(lca)))] <- paste0("LCspeFiltNMF_",c(1:53))
...
quantile(pltdat$scaled.avg,c(0.02,0.98)) # -0.77, 1.02
range(pltdat$scaled.avg) # -1.86, 1.75

png("plots/10_subclustering/07thru10_ver2_53factorNMF/10c-speLCnmfPrefilt_allfactors_projectToLuskinMainClusts.png",height=1000,width=8000)
...
png("plots/10_subclustering/07thru10_ver2_53factorNMF/10c-speLCnmfPrefilt_GSEAdFactors_projectToLuskinMainClusts.png",height=1000,width=1500)
...
## well, most of the factors are in neuronal clusters, with a couple showing esp. high values for glia/mural. 
## One VERY interesting one here though is NMF factor 13, which have a high projection score for vGAT_neurons_5 (and ~0 for DBH!!)
## A quick pass of these through enrichr shows cell types from pan-tissue atlases including endometrium, fallopian tube, adipose; from brain, SST neurons (!!)
## I wonder if this factor is enriched for sex DEGs
sexderes <- readRDS("processed-data/12_DEanalyses_removedsampsAndFinalNMseg/02b-voomLmFit_objs_and_DE_tables_LabSex_LabE2vE4_LabSingleGenos_LabAnces_LabAncesE4E2_noMixClusts_YRIcontAndCat_JUL2025.RDS")
sexderes <- sexderes$cont_ances$sexlabel$detabs$LC_Sex
nrow(sexderes[adj.P.Val<0.1&gene_name %in% factspec[factor=="LCnmf13",gene_name]]) # 29
nrow(sexderes[adj.P.Val<0.1]) # 1609...
nrow(sexderes[adj.P.Val<0.1 & gene_name %in% unique(factspec$gene_name)]) #168
# so fisher test for sex de overlap with this nmf factor vs all nmf factors:
fisher.test(matrix(c(29,(168-29),(1609-29), (nrow(sexderes)-168-(1609-29))),nrow=2))
# nope.
```
