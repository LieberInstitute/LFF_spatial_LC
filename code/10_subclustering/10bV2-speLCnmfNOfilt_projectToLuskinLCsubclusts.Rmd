---
title: "10b-speLCnmfNOfilt_projectToLuskinLCsubclusts"
...
## we already did the opposite a couple ways -- NMF on the Luskin single-cell and projection into our spatial data or subclustering of the LC spots in our spatial data; now we're doing the inverse. NMF of LC spots --> project into the single-cell data.
...
## load NO-count-filtering LC NMF output
```{r}
...
mhorth <- mhorth[initial_alias %in% rownames(mslc) & ortholog_ensg %in% rownames(lcnmf$w)]
...
lffprojinto.ms <- RcppML::project(data=logcounts(mslc),w=orthw,L2=0.00005)
lffprojinto.ms <- t(lffprojinto.ms)


stopifnot(nrow(lffprojinto.ms)==ncol(mslc))
rownames(lffprojinto.ms) <- colnames(mslc)
colnames(lffprojinto.ms) <- paste0("V",c(1:53))
## append to sce
colData(mslc) <- cbind(colData(mslc),lffprojinto.ms)
colnames(colData(mslc))[c(11:ncol(colData(mslc)))] <- paste0("LCspeNOfiltNMF_",c(1:53))

usefulname <- colnames(colData(mslc))[11:ncol(colData(mslc))]

seed3 = as.matrix(colData(mslc)[,usefulname])
seed3 = seed3>0
d3 = cbind.data.frame(msclust=as.data.frame(colData(mslc))[,"ident"],
                      seed3) %>% 
...
seed4 = as.matrix(colData(mslc)[,usefulname])
seed4 = apply(seed4, 2, scale)
d4 = cbind.data.frame(msclust=as.data.frame(colData(mslc))[,"ident"],
                      seed4) %>% 
  group_by(msclust) %>%
...
pltdat[,msclust:=factor(msclust,rev(paste0("NE_",c(1:11))))]
pltdat <- pltdat[!is.na(scaled.avg)]
### 2-98%ile range is -1.35, 1 but minmax is -2.1, 2.3, so set limits to -1.5, 1.5 for better color gradation (and also set midpoint to 0 since that's the biologically meaningful point of no projection). ADJUST THE ACTUAL SCALED AVG values to be -1.5, 1.2 for visual interpretability.

pltdat[lessOrEqual(scaled.avg,-1.35),scaled.avg:=-1.5]
pltdat[greaterOrEqual(scaled.avg,1.05),scaled.avg:=1.2]

png("plots/10_subclustering/07thru10_ver2_53factorNMF/10b-speLCnmfNOfilt_allfactors_projectToLuskinLCsubclusts.png",height=1000,width=8000)
ggplot(pltdat, aes(x=nmf, y=msclust, size=prop, color=scaled.avg))+
  geom_count()+
  theme_bw()+
  scale_size(range=c(0,3))+
  scale_color_gradient2(low = "darkblue",mid="white",high="red",midpoint = 0,limits=c(-1.55,1.25),na.value = NA)+
  labs(x="Spatial LC NMF Factor",y="Ms LC SUBcluster")+
...
png("plots/10_subclustering/07thru10_ver2_53factorNMF/10b-speLCnmfNOfilt_GSEAdFactors_projectToLuskinLCsubclusts.png",height=1000,width=1500)
...
## interesting--the filtered NMF factors basically all had at least a non-negative projection avg value and were represented in all cells; here, all are represented in most/all cells, but beyond NE_1-3 (which seemed to be the only believable-looking singlet NE cells from this data IMO), many of the averages are negative. so perhaps that indicates greater LC specificity for some of these factors identified using unfiltered data?
```
