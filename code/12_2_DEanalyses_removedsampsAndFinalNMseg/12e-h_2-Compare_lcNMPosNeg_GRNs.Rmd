...
rnets <- lapply(rnets,function(x){
    ...
    x[,nmp_spec:=ifelse(nmp_pn*pnorm(nmp-nmn)>0.75,yes="Y",no="N")]
    x[,nmn_spec:=ifelse(nmn_pn*pnorm(nmn-nmp)>0.75,yes="Y",no="N")]
...
rnetspec <- lapply(rnets,function(x){
    ...
    y <- y[!(nmp_spec=="Y"&nmn_spec=="Y")]# remove edges that score as specific to both networks, because that's obv not specific (and shouldn't happen by the calculations anyhow)
    return(y)
})
...
# this is agnostic to TF repressive/enhancing effect because the edge weights are just Z scores of how likely SOME interaction is (not a specific direction of gene regulation) (i.e., if TF does some of each, it can still be included if vast majority of edges are specific to one network)
```{r}
...
```{r}
topdifnet <- rnetspec[diff>abs(nmp)|diff>abs(nmn)|diff>5][nmp>2|nmn>2]
# MLX -> adipoR2 is an interesting NM+ specific, large-difference hit that doesn't pass the diff>NMP threshold, but because the NMP edge score is high
topdifnet <- as.data.table(rbind(topdifnet,rnetspec[gene=="ADIPOR2"&TF=="MLX"]))
...
# lol okay just autoregulation ^. so drop the MLX-as-target-rows
...
targnmpprop <- nrow(mlxplot[lgwt>0])/nrow(mlxplot)
## 0.778 of MLX's target edges are positive

# get activer-subdomain-proportionate number of top 10% highest edge weights, so
# 0.9 + (0.1 * (1-targnmpprop)) positive edges: >92.2ish %ile
# 0 + (0.1*(1-targnmpprop)) negative edges: <7.8ish %ile.
# something about working ^this out threw me for a massive fucking loop but the math works so okay 
mlxplot <- rbind(
    mlxplot[lgwt>quantile(lgwt,(0.9+(0.1*(1-targnmpprop))))],
    mlxplot[lgwt<quantile(lgwt,(0.1*(1-targnmpprop)))]
)
