---
title: "02-PCA, UMAP, Harmony Plots by Cohort Variables"
author: "Bernard Mulvey"
date: "2024-07-24"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(fig.height = 10,fig.width = 7,include = FALSE)
knitr::opts_chunk$set(fig.width=7,fig.height=10)
#### sets tab autocompletion for directories to begin from the directory containing the .Rproj session, instead of the script's directory if different
knitr::opts_knit$set(root.dir = here::here())

library(data.table)
library(Biostrings)
library(ggplot2)
library(gridExtra)
library(ggrastr)
library(SpatialExperiment)
require(colorout)
library(BiocParallel)

ColorOut()
options("styler.addins_style_transformer" = "biocthis::bioc_style()")
library(gtools)

## change write.table to output a TSV without quotes or rownames by default
library(default)
default(write.table) <- list(row.names=F,col.names=T,sep='\t',quote=F)

## enable forked parallel processing with BiocParallel::multicoreParam, future::, etc. seemed to need this a couple times, but otherwise havent so its here as a preventative measure.
library(parallelly)
options(parallelly.supportsMulticore.disableOn="")
options(parallelly.fork.enable=TRUE)
library(BiocParallel)
options(bphost="localhost")

theme_set(theme_bw()+theme(axis.text.x = element_text(size = 14), axis.title.x = element_text(size = 16), axis.text.y = element_text(size = 14), axis.title.y = element_text(size =16), plot.title = element_text(size = 20,hjust=0.5), strip.text = element_text(size=18), legend.text = element_text(size=10), legend.title = element_text(size=11,hjust=0.5)))
```

```{r}
# load SPE--already contains the additional demographic/technical data at this point (age, sex, BMI, PMI, best RIN from cortex..), with samples rotated
lc <- readRDS("processed-data/05_QC/04-lffLC_noEdges_noLocOutliers_remainder1pctileUMIorGene-removed.RDS")

# we only need the colData with spot identifiers here so
lccd <- colData(lc)
rm(lc)
gc(full=T)
# load dimensionality reductions
dimredlist <- readRDS("processed-data/06_FeatSelection_Dimred_Harmony/02-LCfiltered_2HVGsets_dimredslot.RDS")


# look thru  variables we want to plot
colnames(lccd)

# for dimensionality reduction at the spot level, we want to include brnum, agedeath, race, sex, PMI, best_rin_pfc, bmi, sample_id, sum_umi, sum_gene, expr_chrM, slide, and to be sure nothing too strange is happening, array_row and array_col
labvars <- c("sum_umi","sum_gene","expr_chrM_ratio","APOE","Ancestry","Sex","Age","Rin","Visium_slide","brnum","Kit")
```

```{r}
bpmapply(X=dimredlist,Y=names(dimredlist),SIMPLIFY=FALSE,BPPARAM=SnowParam(workers = 8,type="SOCK"),BPOPTIONS=bpoptions(exports=c("labvars","lccd"),packages = c("ggplot2","SpatialExperiment","gridExtra","data.table","ggrastr")),FUN=function(X,Y){
    # dimred combinations to plot #
    if(length(grep(Y,pattern="UMAP",value=T))>0){
        zq <- rbind(c(1,2))
    }
    else{
        zq <- rbind(c(1,2),c(1,3),c(2,3))
    }
    
    
    
    v <- 1
    for (v in c(1:length(labvars))){
        plts <- list()
        m<-1
        for (m in c(1:nrow(zq))){
            z <- zq[m,1]
            q <- zq[m,2]
            pltdat <- cbind(X[,c(z,q)],lccd[,labvars[v]])
            
            pltdat <- as.data.table(pltdat)
            setnames(pltdat,c(paste0(Y,paste0("_Dim",z)),paste0(Y,paste0("_Dim",q)),labvars[v]))
            xvar <- names(pltdat)[1]
            yvar <- names(pltdat)[2]
            
            # set dimensions to numeric, plotting variable to factor or numeric
            set(pltdat,j=xvar,value=as.numeric(pltdat[,get(xvar)]))
            set(pltdat,j=yvar,value=as.numeric(pltdat[,get(yvar)]))
            if(labvars[v] %in% c("sum_umi","sum_gene","expr_chrM_ratio","Age","Rin")){
                set(pltdat,j=labvars[v],value=as.numeric(pltdat[,get(labvars[v])]))
            }
            else{
                set(pltdat,j=labvars[v],value=as.factor(pltdat[,get(labvars[v])]))
            }
     
            plts[[length(plts)+1]] <- ggplot(pltdat,aes(x=.data[[xvar]],y=.data[[yvar]],col=.data[[labvars[v]]]))+
                rasterise(geom_point(size=0.035),dpi=150)
                #scale_color_discrete(DiscretePalette_scCustomize(num_colors = 2 + length(unique(colData(lc.tmp)$label)), palette = "varibow")[3:(length(unique(colData(lc.tmp)$label)) + 2)])
        }
    
    ## determine appropriate subfolder for pllots -- github copilot came out swinging and filled most of this in near-perfectly!
       if (length(grep(Y,pattern="PCA",value=T))>0){subdir<-"02-PCA/"}
       else if (length(grep(Y,pattern="HARMONYdflt",value=T))>0){subdir<-"02-Harmonydefault/"}
       else if (length(grep(Y,pattern="HARMONYlmbna",value=T))>0){subdir<-"02-Harmonylambdanull/"}
       else if(length(grep(Y,pattern="mnn30",value=T))>0){subdir<-"02-mnn30/"}
       else{subdir<-"02-UMAP/"}
        
       if(nrow(zq)>1){
          png(paste0("plots/06_FeatSelection_Dimred_Harmony/",subdir,Y,"_by_",labvars[v],".png"),width=15,height=15*nrow(zq),units="in",res=150)
          do.call("grid.arrange",plts)
       } else{
          png(paste0("plots/06_FeatSelection_Dimred_Harmony/",subdir,Y,"_by_",labvars[v],".png"),width=15,height=15,units="in",res=150)
          print(plts[[1]])
       }
       
       dev.off()
       rm(plts,pltdat,xvar,yvar)
    }
})

```

```{r}
sessionInfo()
sessioninfo::session_info()
```
R version 4.4.1 RC (2024-06-06 r86719)
Platform: aarch64-apple-darwin20
Running under: macOS Sonoma 14.5

Matrix products: default
BLAS:   /System/Library/Frameworks/Accelerate.framework/Versions/A/Frameworks/vecLib.framework/Versions/A/libBLAS.dylib 
LAPACK: /Library/Frameworks/R.framework/Versions/4.4-arm64/Resources/lib/libRlapack.dylib;  LAPACK version 3.12.0

locale:
[1] en_US.UTF-8/en_US.UTF-8/en_US.UTF-8/C/en_US.UTF-8/en_US.UTF-8

time zone: America/Chicago
tzcode source: internal

attached base packages:
[1] stats4    stats     graphics  grDevices utils     datasets  methods   base     

other attached packages:
 [1] ggrastr_1.0.2                  gtools_3.9.5                  
 [3] default_1.0.0                  harmony_1.2.0                 
 [5] Rcpp_1.0.13                    batchelor_1.20.0              
 [7] spatialLIBD_1.16.2             Biostrings_2.72.1             
 [9] XVector_0.44.0                 gridExtra_2.3                 
[11] data.table_1.15.4              BiocParallel_1.38.0           
[13] parallelly_1.37.1              colorout_1.3-0.2              
[15] scater_1.32.0                  ggplot2_3.5.1                 
[17] scran_1.32.0                   scuttle_1.14.0                
[19] SpatialFeatureExperiment_1.6.1 SpatialExperiment_1.14.0      
[21] SingleCellExperiment_1.26.0    SummarizedExperiment_1.34.0   
[23] Biobase_2.64.0                 GenomicRanges_1.56.1          
[25] GenomeInfoDb_1.40.1            IRanges_2.38.1                
[27] S4Vectors_0.42.1               BiocGenerics_0.50.0           
[29] MatrixGenerics_1.16.0          matrixStats_1.3.0             
[31] rlang_1.1.4                   

loaded via a namespace (and not attached):
  [1] bitops_1.0-7              sf_1.0-16                 EBImage_4.46.0           
  [4] httr_1.4.7                RColorBrewer_1.1-3        doParallel_1.0.17        
  [7] tools_4.4.1               utf8_1.2.4                R6_2.5.1                 
 [10] DT_0.33                   ResidualMatrix_1.14.1     HDF5Array_1.32.0         
 [13] lazyeval_0.2.2            uwot_0.2.2                rhdf5filters_1.16.0      
 [16] withr_3.0.0               sp_2.1-4                  cli_3.6.3                
 [19] labeling_0.4.3            sass_0.4.9                proxy_0.4-27             
 [22] Rsamtools_2.20.0          R.utils_2.12.3            sessioninfo_1.2.2        
 [25] attempt_0.3.1             maps_3.4.2                limma_3.60.3             
 [28] rstudioapi_0.16.0         RSQLite_2.3.7             generics_0.1.3           
 [31] BiocIO_1.14.0             spdep_1.3-5               dplyr_1.1.4              
 [34] Matrix_1.7-0              ggbeeswarm_0.7.2          fansi_1.0.6              
 [37] abind_1.4-5               R.methodsS3_1.8.2         terra_1.7-78             
 [40] lifecycle_1.0.4           yaml_2.3.9                edgeR_4.2.0              
 [43] rhdf5_2.48.0              SparseArray_1.4.8         BiocFileCache_2.12.0     
 [46] paletteer_1.6.0           grid_4.4.1                blob_1.2.4               
 [49] promises_1.3.0            dqrng_0.4.1               ExperimentHub_2.12.0     
 [52] crayon_1.5.3              lattice_0.22-6            beachmat_2.20.0          
 [55] cowplot_1.1.3             KEGGREST_1.44.1           magick_2.8.3             
 [58] zeallot_0.1.0             pillar_1.9.0              knitr_1.48               
 [61] metapod_1.12.0            rjson_0.2.21              boot_1.3-30              
 [64] codetools_0.2-20          wk_0.9.2                  glue_1.7.0               
 [67] vctrs_0.6.5               png_0.1-8                 spam_2.10-0              
 [70] gtable_0.3.5              rematch2_2.1.2            cachem_1.1.0             
 [73] xfun_0.45                 S4Arrays_1.4.1            mime_0.12                
 [76] DropletUtils_1.24.0       sfheaders_0.4.4           iterators_1.0.14         
 [79] units_0.8-5               fields_16.2               statmod_1.5.0            
 [82] bluster_1.14.0            bit64_4.0.5               filelock_1.0.3           
 [85] RcppAnnoy_0.0.22          rprojroot_2.0.4           bslib_0.7.0              
 [88] irlba_2.3.5.1             vipor_0.4.7               KernSmooth_2.23-24       
 [91] colorspace_2.1-0          spData_2.3.1              DBI_1.2.3                
 [94] tidyselect_1.2.1          bit_4.0.5                 compiler_4.4.1           
 [97] curl_5.2.1                BiocNeighbors_1.22.0      DelayedArray_0.30.1      
[100] plotly_4.10.4             rtracklayer_1.64.0        scales_1.3.0             
[103] classInt_0.4-10           rappdirs_0.3.3            tiff_0.1-12              
[106] digest_0.6.36             fftwtools_0.9-11          rmarkdown_2.27           
[109] benchmarkmeData_1.0.4     RhpcBLASctl_0.23-42       htmltools_0.5.8.1        
[112] pkgconfig_2.0.3           jpeg_0.1-10               sparseMatrixStats_1.16.0 
[115] dbplyr_2.5.0              fastmap_1.2.0             htmlwidgets_1.6.4        
[118] UCSC.utils_1.0.0          shiny_1.8.1.1             DelayedMatrixStats_1.26.0
[121] farver_2.1.2              jquerylib_0.1.4           jsonlite_1.8.8           
[124] config_0.3.2              R.oo_1.26.0               BiocSingular_1.20.0      
[127] RCurl_1.98-1.14           magrittr_2.0.3            GenomeInfoDbData_1.2.12  
[130] s2_1.1.6                  dotCall64_1.1-1           Rhdf5lib_1.26.0          
[133] munsell_0.5.1             viridis_0.6.5             zlibbioc_1.50.0          
[136] AnnotationHub_3.12.0      parallel_4.4.1            ggrepel_0.9.5            
[139] deldir_2.0-4              locfit_1.5-9.10           igraph_2.0.3             
[142] ScaledMatrix_1.12.0       BiocVersion_3.19.1        XML_3.99-0.17            
[145] evaluate_0.24.0           golem_0.4.1               BiocManager_1.30.23      
[148] foreach_1.5.2             httpuv_1.6.15             tidyr_1.3.1              
[151] purrr_1.0.2               benchmarkme_1.0.8         rsvd_1.0.5               
[154] xtable_1.8-4              restfulr_0.0.15           e1071_1.7-14             
[157] later_1.3.2               viridisLite_0.4.2         class_7.3-22             
[160] snow_0.4-4                tibble_3.2.1              memoise_2.0.1            
[163] beeswarm_0.4.0            AnnotationDbi_1.66.0      GenomicAlignments_1.40.0 
[166] cluster_2.1.6             shinyWidgets_0.8.6        here_1.0.1               
> sessioninfo::session_info()
─ Session info ───────────────────────────────────────────────────────────────────
 setting  value
 version  R version 4.4.1 RC (2024-06-06 r86719)
 os       macOS Sonoma 14.5
 system   aarch64, darwin20
 ui       RStudio
 language (EN)
 collate  en_US.UTF-8
 ctype    en_US.UTF-8
 tz       America/Chicago
 date     2024-07-24
 rstudio  2024.07.0-daily+219 Cranberry Hibiscus (desktop)
 pandoc   3.1.11 @ /Applications/RStudio.app/Contents/Resources/app/quarto/bin/tools/aarch64/ (via rmarkdown)

─ Packages ───────────────────────────────────────────────────────────────────────
 ! package                  * version   date (UTC) lib source
   abind                      1.4-5     2016-07-21 [1] CRAN (R 4.4.0)
   AnnotationDbi              1.66.0    2024-05-01 [1] Bioconductor 3.19 (R 4.4.0)
   AnnotationHub              3.12.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   attempt                    0.3.1     2020-05-03 [1] CRAN (R 4.4.0)
   batchelor                * 1.20.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   beachmat                   2.20.0    2024-05-06 [1] Bioconductor 3.19 (R 4.4.0)
   beeswarm                   0.4.0     2021-06-01 [1] CRAN (R 4.4.0)
   benchmarkme                1.0.8     2022-06-12 [1] CRAN (R 4.4.0)
   benchmarkmeData            1.0.4     2020-04-23 [1] CRAN (R 4.4.0)
   Biobase                  * 2.64.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   BiocFileCache              2.12.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   BiocGenerics             * 0.50.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   BiocIO                     1.14.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   BiocManager                1.30.23   2024-05-04 [1] CRAN (R 4.4.0)
   BiocNeighbors              1.22.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   BiocParallel             * 1.38.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.1)
   BiocSingular               1.20.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   BiocVersion                3.19.1    2024-04-22 [1] Bioconductor 3.19 (R 4.4.0)
   Biostrings               * 2.72.1    2024-06-02 [1] Bioconductor 3.19 (R 4.4.0)
   bit                        4.0.5     2022-11-15 [1] CRAN (R 4.4.0)
   bit64                      4.0.5     2020-08-30 [1] CRAN (R 4.4.0)
   bitops                     1.0-7     2021-04-24 [1] CRAN (R 4.4.0)
   blob                       1.2.4     2023-03-17 [1] CRAN (R 4.4.0)
   bluster                    1.14.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   boot                       1.3-30    2024-02-26 [1] CRAN (R 4.4.1)
   bslib                      0.7.0     2024-03-29 [1] CRAN (R 4.4.0)
   cachem                     1.1.0     2024-05-16 [1] CRAN (R 4.4.0)
   class                      7.3-22    2023-05-03 [1] CRAN (R 4.4.1)
   classInt                   0.4-10    2023-09-05 [1] CRAN (R 4.4.0)
 P cli                        3.6.3     2024-06-21 [2] CRAN (R 4.4.0)
   cluster                    2.1.6     2023-12-01 [1] CRAN (R 4.4.1)
   codetools                  0.2-20    2024-03-31 [1] CRAN (R 4.4.1)
   colorout                 * 1.3-0.2   2024-05-01 [1] Github (jalvesaq/colorout@c6113a2)
   colorspace                 2.1-0     2023-01-23 [1] CRAN (R 4.4.0)
   config                     0.3.2     2023-08-30 [1] CRAN (R 4.4.0)
   cowplot                    1.1.3     2024-01-22 [1] CRAN (R 4.4.0)
   crayon                     1.5.3     2024-06-20 [1] CRAN (R 4.4.0)
   curl                       5.2.1     2024-03-01 [1] CRAN (R 4.4.0)
   data.table               * 1.15.4    2024-03-30 [2] CRAN (R 4.4.0)
   DBI                        1.2.3     2024-06-02 [1] CRAN (R 4.4.0)
   dbplyr                     2.5.0     2024-03-19 [1] CRAN (R 4.4.0)
   default                  * 1.0.0     2017-08-07 [1] CRAN (R 4.4.0)
   DelayedArray               0.30.1    2024-05-07 [1] Bioconductor 3.19 (R 4.4.0)
   DelayedMatrixStats         1.26.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   deldir                     2.0-4     2024-02-28 [1] CRAN (R 4.4.0)
   digest                     0.6.36    2024-06-23 [1] CRAN (R 4.4.0)
   doParallel                 1.0.17    2022-02-07 [1] CRAN (R 4.4.0)
   dotCall64                  1.1-1     2023-11-28 [1] CRAN (R 4.4.0)
   dplyr                      1.1.4     2023-11-17 [1] CRAN (R 4.4.0)
   dqrng                      0.4.1     2024-05-28 [1] CRAN (R 4.4.0)
   DropletUtils               1.24.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   DT                         0.33      2024-04-04 [1] CRAN (R 4.4.0)
   e1071                      1.7-14    2023-12-06 [1] CRAN (R 4.4.0)
   EBImage                    4.46.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   edgeR                      4.2.0     2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   evaluate                   0.24.0    2024-06-10 [1] CRAN (R 4.4.0)
   ExperimentHub              2.12.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   fansi                      1.0.6     2023-12-08 [1] CRAN (R 4.4.0)
   farver                     2.1.2     2024-05-13 [1] CRAN (R 4.4.0)
   fastmap                    1.2.0     2024-05-15 [1] CRAN (R 4.4.0)
   fftwtools                  0.9-11    2021-03-01 [1] CRAN (R 4.4.0)
   fields                     16.2      2024-06-27 [1] CRAN (R 4.4.0)
   filelock                   1.0.3     2023-12-11 [1] CRAN (R 4.4.0)
   foreach                    1.5.2     2022-02-02 [1] CRAN (R 4.4.0)
   generics                   0.1.3     2022-07-05 [1] CRAN (R 4.4.0)
   GenomeInfoDb             * 1.40.1    2024-05-24 [1] Bioconductor 3.19 (R 4.4.0)
   GenomeInfoDbData           1.2.12    2024-05-01 [1] Bioconductor
   GenomicAlignments          1.40.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   GenomicRanges            * 1.56.1    2024-06-12 [1] Bioconductor 3.19 (R 4.4.1)
   ggbeeswarm                 0.7.2     2023-04-29 [1] CRAN (R 4.4.0)
   ggplot2                  * 3.5.1     2024-04-23 [1] CRAN (R 4.4.0)
   ggrastr                  * 1.0.2     2023-06-01 [1] CRAN (R 4.4.0)
   ggrepel                    0.9.5     2024-01-10 [1] CRAN (R 4.4.0)
   glue                       1.7.0     2024-01-09 [1] CRAN (R 4.4.0)
   golem                      0.4.1     2023-06-05 [1] CRAN (R 4.4.0)
   gridExtra                * 2.3       2017-09-09 [1] CRAN (R 4.4.0)
   gtable                     0.3.5     2024-04-22 [1] CRAN (R 4.4.0)
   gtools                   * 3.9.5     2023-11-20 [1] CRAN (R 4.4.0)
   harmony                  * 1.2.0     2023-11-29 [1] CRAN (R 4.4.0)
   HDF5Array                  1.32.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   here                       1.0.1     2020-12-13 [1] CRAN (R 4.4.0)
   htmltools                  0.5.8.1   2024-04-04 [1] CRAN (R 4.4.0)
   htmlwidgets                1.6.4     2023-12-06 [1] CRAN (R 4.4.0)
   httpuv                     1.6.15    2024-03-26 [1] CRAN (R 4.4.0)
   httr                       1.4.7     2023-08-15 [1] CRAN (R 4.4.0)
   igraph                     2.0.3     2024-03-13 [1] CRAN (R 4.4.0)
   IRanges                  * 2.38.1    2024-07-03 [1] Bioconductor 3.19 (R 4.4.1)
   irlba                      2.3.5.1   2022-10-03 [1] CRAN (R 4.4.0)
   iterators                  1.0.14    2022-02-05 [1] CRAN (R 4.4.0)
   jpeg                       0.1-10    2022-11-29 [1] CRAN (R 4.4.0)
   jquerylib                  0.1.4     2021-04-26 [1] CRAN (R 4.4.0)
   jsonlite                   1.8.8     2023-12-04 [1] CRAN (R 4.4.0)
   KEGGREST                   1.44.1    2024-06-19 [1] Bioconductor 3.19 (R 4.4.0)
   KernSmooth                 2.23-24   2024-05-17 [1] CRAN (R 4.4.1)
   knitr                      1.48      2024-07-07 [1] CRAN (R 4.4.1)
   labeling                   0.4.3     2023-08-29 [1] CRAN (R 4.4.0)
   later                      1.3.2     2023-12-06 [1] CRAN (R 4.4.0)
   lattice                    0.22-6    2024-03-20 [1] CRAN (R 4.4.1)
   lazyeval                   0.2.2     2019-03-15 [1] CRAN (R 4.4.0)
   lifecycle                  1.0.4     2023-11-07 [1] CRAN (R 4.4.0)
   limma                      3.60.3    2024-06-16 [1] Bioconductor 3.19 (R 4.4.0)
   locfit                     1.5-9.10  2024-06-24 [1] CRAN (R 4.4.0)
   magick                     2.8.3     2024-02-18 [1] CRAN (R 4.4.0)
   magrittr                   2.0.3     2022-03-30 [1] CRAN (R 4.4.0)
   maps                       3.4.2     2023-12-15 [1] CRAN (R 4.4.0)
   Matrix                     1.7-0     2024-04-26 [1] CRAN (R 4.4.1)
   MatrixGenerics           * 1.16.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   matrixStats              * 1.3.0     2024-04-11 [1] CRAN (R 4.4.0)
   memoise                    2.0.1     2021-11-26 [1] CRAN (R 4.4.0)
   metapod                    1.12.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   mime                       0.12      2021-09-28 [1] CRAN (R 4.4.0)
   munsell                    0.5.1     2024-04-01 [1] CRAN (R 4.4.0)
   paletteer                  1.6.0     2024-01-21 [1] CRAN (R 4.4.0)
   parallelly               * 1.37.1    2024-02-29 [1] CRAN (R 4.4.0)
   pillar                     1.9.0     2023-03-22 [1] CRAN (R 4.4.0)
   pkgconfig                  2.0.3     2019-09-22 [1] CRAN (R 4.4.0)
   plotly                     4.10.4    2024-01-13 [1] CRAN (R 4.4.0)
   png                        0.1-8     2022-11-29 [1] CRAN (R 4.4.0)
   promises                   1.3.0     2024-04-05 [1] CRAN (R 4.4.0)
   proxy                      0.4-27    2022-06-09 [1] CRAN (R 4.4.0)
   purrr                      1.0.2     2023-08-10 [1] CRAN (R 4.4.0)
   R.methodsS3                1.8.2     2022-06-13 [1] CRAN (R 4.4.0)
   R.oo                       1.26.0    2024-01-24 [1] CRAN (R 4.4.0)
   R.utils                    2.12.3    2023-11-18 [1] CRAN (R 4.4.0)
   R6                         2.5.1     2021-08-19 [1] CRAN (R 4.4.0)
   rappdirs                   0.3.3     2021-01-31 [1] CRAN (R 4.4.0)
   RColorBrewer               1.1-3     2022-04-03 [1] CRAN (R 4.4.0)
   Rcpp                     * 1.0.13    2024-07-17 [1] CRAN (R 4.4.1)
   RcppAnnoy                  0.0.22    2024-01-23 [1] CRAN (R 4.4.0)
   RCurl                      1.98-1.14 2024-01-09 [1] CRAN (R 4.4.0)
   rematch2                   2.1.2     2020-05-01 [1] CRAN (R 4.4.0)
   ResidualMatrix             1.14.1    2024-06-23 [1] Bioconductor 3.19 (R 4.4.0)
   restfulr                   0.0.15    2022-06-16 [1] CRAN (R 4.4.0)
   rhdf5                      2.48.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   rhdf5filters               1.16.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   Rhdf5lib                   1.26.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   RhpcBLASctl                0.23-42   2023-02-11 [1] CRAN (R 4.4.0)
   rjson                      0.2.21    2022-01-09 [1] CRAN (R 4.4.0)
 P rlang                    * 1.1.4     2024-06-04 [2] CRAN (R 4.4.1)
   rmarkdown                  2.27      2024-05-17 [1] CRAN (R 4.4.0)
   rprojroot                  2.0.4     2023-11-05 [1] CRAN (R 4.4.0)
   Rsamtools                  2.20.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   RSQLite                    2.3.7     2024-05-27 [1] CRAN (R 4.4.0)
   rstudioapi                 0.16.0    2024-03-24 [1] CRAN (R 4.4.0)
   rsvd                       1.0.5     2021-04-16 [1] CRAN (R 4.4.0)
   rtracklayer                1.64.0    2024-05-06 [1] Bioconductor 3.19 (R 4.4.0)
   s2                         1.1.6     2023-12-19 [1] CRAN (R 4.4.0)
   S4Arrays                   1.4.1     2024-05-20 [1] Bioconductor 3.19 (R 4.4.0)
   S4Vectors                * 0.42.1    2024-07-03 [1] Bioconductor 3.19 (R 4.4.1)
   sass                       0.4.9     2024-03-15 [1] CRAN (R 4.4.0)
   ScaledMatrix               1.12.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   scales                     1.3.0     2023-11-28 [1] CRAN (R 4.4.0)
   scater                   * 1.32.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   scran                    * 1.32.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   scuttle                  * 1.14.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   sessioninfo                1.2.2     2021-12-06 [1] CRAN (R 4.4.0)
   sf                         1.0-16    2024-03-24 [1] CRAN (R 4.4.0)
   sfheaders                  0.4.4     2024-01-17 [1] CRAN (R 4.4.0)
   shiny                      1.8.1.1   2024-04-02 [1] CRAN (R 4.4.0)
   shinyWidgets               0.8.6     2024-04-24 [1] CRAN (R 4.4.0)
   SingleCellExperiment     * 1.26.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   snow                       0.4-4     2021-10-27 [1] CRAN (R 4.4.0)
   sp                         2.1-4     2024-04-30 [1] CRAN (R 4.4.0)
   spam                       2.10-0    2023-10-23 [1] CRAN (R 4.4.0)
   SparseArray                1.4.8     2024-05-30 [1] Bioconductor 3.19 (R 4.4.0)
   sparseMatrixStats          1.16.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   SpatialExperiment        * 1.14.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   SpatialFeatureExperiment * 1.6.1     2024-05-15 [1] Bioconductor 3.19 (R 4.4.0)
   spatialLIBD              * 1.16.2    2024-05-28 [1] Bioconductor 3.19 (R 4.4.0)
   spData                     2.3.1     2024-05-31 [1] CRAN (R 4.4.0)
   spdep                      1.3-5     2024-06-10 [1] CRAN (R 4.4.0)
   statmod                    1.5.0     2023-01-06 [1] CRAN (R 4.4.0)
   SummarizedExperiment     * 1.34.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   terra                      1.7-78    2024-05-22 [1] CRAN (R 4.4.0)
   tibble                     3.2.1     2023-03-20 [1] CRAN (R 4.4.0)
   tidyr                      1.3.1     2024-01-24 [1] CRAN (R 4.4.0)
   tidyselect                 1.2.1     2024-03-11 [1] CRAN (R 4.4.0)
   tiff                       0.1-12    2023-11-28 [1] CRAN (R 4.4.0)
   UCSC.utils                 1.0.0     2024-05-06 [1] Bioconductor 3.19 (R 4.4.0)
   units                      0.8-5     2023-11-28 [1] CRAN (R 4.4.0)
   utf8                       1.2.4     2023-10-22 [1] CRAN (R 4.4.0)
   uwot                       0.2.2     2024-04-21 [1] CRAN (R 4.4.0)
   vctrs                      0.6.5     2023-12-01 [1] CRAN (R 4.4.0)
   vipor                      0.4.7     2023-12-18 [1] CRAN (R 4.4.0)
   viridis                    0.6.5     2024-01-29 [1] CRAN (R 4.4.0)
   viridisLite                0.4.2     2023-05-02 [1] CRAN (R 4.4.0)
   withr                      3.0.0     2024-01-16 [1] CRAN (R 4.4.0)
   wk                         0.9.2     2024-07-09 [1] CRAN (R 4.4.0)
   xfun                       0.45      2024-06-16 [1] CRAN (R 4.4.0)
   XML                        3.99-0.17 2024-06-25 [1] CRAN (R 4.3.3)
   xtable                     1.8-4     2019-04-21 [1] CRAN (R 4.4.0)
   XVector                  * 0.44.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)
   yaml                       2.3.9     2024-07-05 [1] CRAN (R 4.4.0)
   zeallot                    0.1.0     2018-01-28 [1] CRAN (R 4.4.0)
   zlibbioc                   1.50.0    2024-04-30 [1] Bioconductor 3.19 (R 4.4.0)

 [1] /Library/Frameworks/R.framework/Versions/4.4-arm64/Resources/library
 [2] /Users/bmulvey/Library/R/arm64/4.4/library

 P ── Loaded and on-disk path mismatch.

──────────────────────────────────────────────────────────────────────────────────
