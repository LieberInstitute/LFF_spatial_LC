---
title: "01_02-LCsampsonlyclust_WeberDivecha_registration_plots"
...
## ggplot defaults
theme_set(theme_bw()+theme(axis.text.x = element_text(size = 8,angle=90,hjust=0.5), axis.title.x = element_text(size = 9), axis.text.y = element_text(size = 8), axis.title.y = element_text(size =9), plot.title = element_markdown(size = 11,hjust=0.5), strip.text = element_text(size=11), legend.text = element_text(size=8), legend.title = element_text(size=9,hjust=0.5)))
...
```{r}
regs <- readRDS("processed-data/13_2_markers_and_registration_final/02_2-LC3_registration_to_WeberDivecha23_final.RDS")
...
plts <- lapply(X=regs2,FUN=function(x){
    pltdat <- x[,variable:=as.character(variable)]
    pltdat <- pltdat[,LFF_clus:=as.character(LFF_clus)]
    pltdat <- pltdat[topNmk==5194] # also did 40, 250, here; 5194 for snseq clusters only
...
lapply(names(plts)[1],function(n){
    pdf(paste0("plots/16_sensitivityanalysis_reproc_wo3donors/Sens2_markers_and_registration_final/",n,"top5194.pdf"),height=8,width=10)
    print(plts[[n]])
    dev.off()
})
```
...
mapply(x=regs2,n=names(regs2),SIMPLIFY=FALSE,FUN=function(x,n){
   ...
   pdf(paste0("plots/16_sensitivityanalysis_reproc_wo3donors/Sens2_markers_and_registration_final/01b_LC3_to_WebDiv_",n,".pdf"),height=8,width=10,onefile=TRUE)
   lapply(plts,print)
   dev.off()
})
```

## others (sanity checking)
```{r}
checkplts <- mapply(x=regs2,n=names(regs2),SIMPLIFY=FALSE,FUN=function(x,n){
   i <- 1
   plts <- list()
   for (cl in unique(x$LFF_clus)){
      if (length(grep(cl,pattern="Oligo",value=T))>0){
         wdcs <- 
      }
      pltdat <- x[LFF_clus==i]
      plts[[i]] <- ggplot(pltdat,aes(x=log10(topNmk),y=value,color=variable))+
...
[118] ggbeeswarm_0.7.3         viridisLite_0.4.3        Biostrings_2.78.0       
[121] lazyeval_0.2.2           Matrix_1.7-4             benchmarkme_1.0.8       
[124] bit64_4.6.0-1            KEGGREST_1.50.0          statmod_1.5.1           
[127] shiny_1.12.1             gridtext_0.1.5           memoise_2.0.1           
[130] bslib_0.10.0             benchmarkmeData_2.0.0    bit_4.6.0               
> sessioninfo::session_info()
─ Session info ────────────────────────────────────────────────────────────────────
 setting  value
