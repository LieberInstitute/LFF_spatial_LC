---
title: "10-UnfiltNMstats_per_cluster"
...
clustab <- merge.data.table(clustab,anno[,.(clusid,plot_anno)],by="clusid")

lc3cd <- merge.data.table(lc3cd,clustab[,.(rn,plot_anno)],by.x="rowname",by.y="rn")
setnames(lc3cd,"plot_anno","origDomain")
```

## make plots of %area NM+ per cluster
```{r}
## calculate proportion of spots with any NM for a cluster
ans <- lc3cd[,.(sum(Prop_NM>0)/.N),by=origDomain]
setnames(ans,"V1","propNMspots")
ans[,propNMspots:=round(propNMspots,digits=3)]
ans[,propNMspots:=100*propNMspots]

lc3cd <- merge.data.table(lc3cd,ans,by="origDomain",all.x=T)
# ggtext errors if newline starts with > symbol)
lc3cd[,Domain:=paste0(origDomain,"<br>(",propNMspots,"% spots >0 NM area) ")]

### histogram
pdf("plots/08_validitycheck_25hdg75svg_louv1/10a-nonzero_NMareas_per_cluster_hist.pdf",width=9,height=9)
ggplot(lc3cd[Prop_NM>0],aes(x=100*Prop_NM))+
    geom_histogram(bins=75)+
    facet_wrap(~Domain,scales="fixed",ncol=3)+
    theme(strip.text=element_markdown(size=9))+
    ylab("Number of Spots")+
    xlab("% spot pixels containing NM\n(where >0)")
dev.off()

rm(ans)
### weirdly, there's a spike for every cluster around 0.002. so try above this instead
ans <- lc3cd[,.(sum(Prop_NM>0.003)/.N),by=origDomain]
setnames(ans,"V1","propNMspots")
ans[,propNMspots:=round(propNMspots,digits=3)]
ans[,propNMspots:=100*propNMspots]
...
lc3cd <- merge.data.table(lc3cd,ans,by="origDomain",all.x=T)
lc3cd[,Domain:=paste0(origDomain,"<br>(",propNMspots,"% spots >0.3% NM area)")]


pdf("plots/08_validitycheck_25hdg75svg_louv1/10b-NMarea_per_cluster_hist_above0.003.pdf",height=9,width=9)
ggplot(lc3cd[Prop_NM>0.003],aes(x=100*Prop_NM))+
    geom_histogram(bins=75)+
    facet_wrap(~Domain,scales="fixed",ncol=3)+
    theme(strip.text=element_markdown(size=9))+
    ylab("Number of Spots")+
...
[15] ggplot2_3.5.2               data.table_1.17.4          
[17] rlang_1.1.5                

loaded via a namespace (and not attached):
 [1] gtable_0.3.6            rjson_0.2.23            xfun_0.52              
 [4] lattice_0.22-6          vctrs_0.6.5             tools_4.4.3            
 [7] generics_0.1.4          tibble_3.2.1            pkgconfig_2.0.3        
[10] Matrix_1.7-3            RColorBrewer_1.1-3      lifecycle_1.0.4        
[13] GenomeInfoDbData_1.2.13 stringr_1.5.1           compiler_4.4.3         
[16] farver_2.1.2            litedown_0.6            htmltools_0.5.8.1      
[19] yaml_2.3.10             pillar_1.10.2           crayon_1.5.3           
[22] DelayedArray_0.32.0     magick_2.8.6            abind_1.4-8            
[25] commonmark_1.9.5        tidyselect_1.2.1        digest_0.6.37          
[28] stringi_1.8.7           dplyr_1.1.4             labeling_0.4.3         
[31] rprojroot_2.0.4         fastmap_1.2.0           grid_4.4.3             
[34] here_1.0.1              cli_3.6.4               SparseArray_1.6.2      
[37] magrittr_2.0.3          S4Arrays_1.6.0          dichromat_2.0-0.1      
[40] withr_3.0.2             scales_1.4.0            UCSC.utils_1.2.0       
[43] rmarkdown_2.29          XVector_0.46.0          httr_1.4.7             
[46] evaluate_1.0.3          knitr_1.50              markdown_2.0           
