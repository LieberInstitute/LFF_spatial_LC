...
e4e2s <- e4e2s[,.(NES,pathway,Domain,Comparison,dir)] # subset to important columns
...
domxprgs <- mclapply(doms,mc.cores=2,FUN=function(D){
    ...
    # calculate mean % spots expressing dataset wide and number of samples with expression over 2%. each tf occurs once per sample, so take the mean across rows with a given TF value for samplewise means, or tabulate the number of TF occurrences with the pctxpr filter to get the number of passing rows
    ...
    # FINALLY  get the genes meeting criteria (expressed in at least 5% of spots of domain for at least 12 samples, or >5% of spots on average across ALL samples with the domain) back and filter the result table to them
...
mapply(g=gseas2,n=names(gseas2),SIMPLIFY=FALSE,function(g,n){
  fwrite(g,paste0("plots/manuscript/SuppTabs/TabDelims/ST-TFtargGSEA_TFin4pctOfDomSpotsOnAvgOrIn12Samps_",n,".txt"),sep='\t',quote=F,row.names=F,col.names=T)
})
```
