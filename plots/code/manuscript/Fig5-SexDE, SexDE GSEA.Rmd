...
## UPDATED 03/18/26, as ACHE_SERT_LC_WM (unsurprisingly) DOES NOT LABEL TRANSFER ONTO XENIUM: Switch to ACHE_SERT_NPEP ##
```{r}
## load cluster palette
pal <- readRDS("plots/finalpal_somuchfornorainbows.RDS")
pal <- pal[c("LC","*ACHE*, *SERT*, Neuropeptide Mixed","Astro")]
names(pal)[2] <- "*ACHE*, *SERT*,<br>Neuropeptide Mixed"
...
## get the results using continuous ancestry
...
names(detabs)[2] <- "*ACHE*, *SERT*,<br>Neuropeptide Mixed"

## identify genes to be labeled in plots
# LC TMC6 APOLD1 CHGB PRCP GSTT2B
# astro HSPB8 GASK1B NTSR2 DNAJB1
# achesertlcwm BAG3 PHYH CPNE6 LRRTM3 ## CHANGED TO ACHE NPEP
### LRRTM3: ncbi synopsis is brief and on point: "Acts upstream of or within positive regulation of amyloid-beta formation."
# achesertnpep C3, CX3CR1, PENK, APOLD1, ADCYAP1, CALCB

detabs$LC[,label:=""]
detabs$LC[gene_name %in% c("TMC6", "CHGB", "TCF20", "APOLD1", "PRCP", "GSTT2B"),label:=gene_name]

detabs$`*ACHE*, *SERT*,<br>Neuropeptide Mixed`[,label:=""]
detabs$`*ACHE*, *SERT*,<br>Neuropeptide Mixed`[gene_name %in% c("C3", "CX3CR1", "PENK", "APOLD1", "ADCYAP1", "CALCB"),label:=gene_name]
...
pltdetabs <- detabs[c("LC","Astro","*ACHE*, *SERT*,<br>Neuropeptide Mixed")]
...
### make plots
fn <- c("LC","*ACHE*, *SERT*,<br>Neuropeptide Mixed","Astro")
...
for (i in c(1:3)){
    ...
    p <- ggplot(pltdetabs[!(chr %in% c("chrX","chrY","chrMT"))&clust==x],aes(x=logFC,y=-log10(P.Value),color=clust)) +
    ...
    if(n=="*ACHE*, *SERT*,<br>Neuropeptide Mixed"){
        p <- p+theme(legend.text=element_markdown(size=7))
        w<-3
        
        p<-rasterize(p,layers="points",dpi=450,dev="cairo_png")
        nout <- gsub(n,pattern="\\*",replacement="")
        nout <- gsub(nout,pattern="<br>",replacement="")
        pdf(paste0("plots/manuscript/Fig5/5ABC-SexDE_Volcano_",nout,".pdf"),width=w,height=2,onefile=FALSE)
        ...
        pdf(paste0("plots/manuscript/Fig5/5ABC-SexDE_Volcano_",n,".pdf"),width=w,height=2,onefile=FALSE)
        print(p)
        dev.off()
    }
}
...
pdf("plots/manuscript/Fig5/5D-LCsexDE_msigdbGSEA.pdf",height=3,width=3.35)
...
gmat <- pbulk$cont_ances$sexlabel$voomLFt$EList$E
# subset to LC
gmat <- gmat[,colnames(gmat) %in% grep(colnames(gmat),pattern="LC_V|^Astro_V",value=T)]
# make symbols
rownames(gmat) <- pbulk$cont_ances$sexlabel$voomLFt$genes$gene_name

# get sample metadata in the same arrangement for LC pbulks
cd <- as.data.frame(pbulk$cont_ances$sexlabel$voomLFt$targets)[colnames(gmat),]
...
## not correctin for astro_F,astro_M,LC_f,LC_M (i.e., first 4 columns of the model matrix)
...
pdf("plots/manuscript/Fig5/5E-Cholestgenes_astro_LC_pbulk.pdf",height=3,width=4)
...
lapply(plotg,FUN=function(g){
    ...
    ## save
    pdf(paste0("plots/manuscript/Fig5/5F-",g,".pdf"),height=2,width=4.5,onefile = FALSE)
