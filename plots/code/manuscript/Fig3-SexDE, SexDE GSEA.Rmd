...
names(detabs)[1] <- "*ACHE*, *SERT*, LC, Oligo Mixed"

detabs$LC[,label:=""]
detabs$LC[gene_name %in% c("DUSP1", "CLDN5", "TCF20", "APOLD1", "NR4A1", "TYROBP","APOC2"),label:=gene_name]

detabs$`*ACHE*, *SERT*, LC, Oligo Mixed`[,label:=""]
detabs$`*ACHE*, *SERT*, LC, Oligo Mixed`[gene_name %in% c("GAD1", "SLC32A1","HSPA5","PSD"," SSTR2","HSF1"),label:=gene_name]

detabs$Astro[,label:=""]
detabs$Astro[gene_name %in% c("ATN1", "MLC1", "MCL1", "FGFR3", "DNAJC12", "HSPA1B", "OLR1"),label:=gene_name]

pltdetabs <- detabs[c("LC","Astro","*ACHE*, *SERT*, LC, Oligo Mixed")]
fn <- c("LC","Astro","ACHE_SERT_LC_Oligo")
...
mapply(x=names(pal),n=fn,SIMPLIFY=FALSE,FUN=function(x,n,clus){
    
    p <- ggplot(x[!(chr %in% c("chrX","chrY","chrMT"))&clust==x],aes(x=logFC,y=-log10(P.Value),col=clust)) +
        geom_point(size=1.2,stroke=0,alpha=0.7) +
    ...
    if(n=="ACHE_SERT_LC_Oligo"){
        ...
        pdf(paste0("plots/manuscript/Fig3/3ABC-SexDE_Volcano_",n,".pdf"),width=w,height=2,onefile=FALSE)
        print(p)
        dev.off()
...
pdf("plots/manuscript/Fig3/3D-LCsexDE_msigdbGSEA.pdf",height=3,width=3.35)
...
pdf("plots/manuscript/Fig3/3E-Cholestgenes_astro_LC_pbulk.pdf",height=3,width=4)
...
lapply(plotg,FUN=function(g){
    ...
    ## save
    pdf(paste0("plots/manuscript/Fig3/3F-",g,".pdf"),height=2,width=4.5,onefile = FALSE)
