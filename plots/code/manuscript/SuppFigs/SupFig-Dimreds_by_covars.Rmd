...
# covars to plot by: 
# biological: APOE, Ancestry, Sex, brnum, Age (continuous)
# technical:  sample_id, capture_id,  sum_gene (cont), expr_chrM_ratio (cont), sum_umi (contin)

dimr <- lapply(dimr,as.data.table,keep.rownames=T)
dimr <- dimr[c("UMAP_HDG_SVG_2575", "HARMONYlmbna_HDG_SVG_2575_UMAP")]

names(dimr) <- c("UMAP","HARMONY-UMAP")
```

## dimred plots
```{r}
## correct APOE labeling
metad[APOE=="E4/ E4",APOE:="E4hom"]
metad[APOE=="E3/ E4",APOE:="E4het"]
metad[APOE=="E2/ E3",APOE:="E2het"]
metad[APOE=="E2/ E2",APOE:="E2hom"]

bcovs <- c("APOE", "Ancestry", "Sex", "brnum", "Age")
bplt <- bpmapply(d=dimr,f=names(dimr),SIMPLIFY=FALSE,BPPARAM = MulticoreParam(4),FUN=function(d,f){
    plotdr <- names(d)[c(1:3)] ## spot id and first 2 PCs/Harmos or 2 UMAP dims
    tmp <- copy(d)[,..plotdr]
    
    mdcol <- c("rowname",bcovs)
    
    tmp <- merge.data.table(tmp,metad[,..mdcol],by.x="rn",by.y="rowname")
    plts <- lapply(bcovs,function(x){
...
pdf(paste0("plots/manuscript/SuppFigs/SupFig-DimredCovarPlots_biocovs.pdf"),width=8,height=10)
do.call("grid.arrange",c(bplt[[1]][1],bplt[[2]][1],bplt[[1]][2],bplt[[2]][2],bplt[[1]][3],bplt[[2]][3],bplt[[1]][4],bplt[[2]][4],bplt[[1]][5],bplt[[2]][5],ncol=2))
dev.off()
...
## tech covars
```{r}
tcovs <- c("sample_id", "capture_id",  "sum_gene", "expr_chrM_ratio", "sum_umi")
tplt <- bpmapply(d=dimr,f=names(dimr),SIMPLIFY=FALSE,BPPARAM = MulticoreParam(4),FUN=function(d,f){
    plotdr <- names(d)[c(1:3)] ## spot id and first 2 PCs/Harmos or 2 UMAP dims
    tmp <- copy(d)[,..plotdr]
    
    mdcol <- c("rowname",tcovs)
    
    tmp <- merge.data.table(tmp,metad[,..mdcol],by.x="rn",by.y="rowname")
    plts <- lapply(tcovs,function(x){
        if(x %in% c("sum_gene", "sum_umi", "expr_chrM_ratio", "Age")){
            xv <- names(tmp)[2]
            yv <- names(tmp)[3]
            p <- ggplot(tmp,aes(x=.data[[xv]],y=.data[[yv]],color=as.numeric(.data[[x]])))+
...
pdf(paste0("plots/manuscript/SuppFigs/SupFig-DimredCovarPlots_techcovs.pdf"),width=8,height=10)
do.call("grid.arrange",c(tplt[[1]][1],tplt[[2]][1],tplt[[1]][2],tplt[[2]][2],tplt[[1]][3],tplt[[2]][3],tplt[[1]][4],tplt[[2]][4],tplt[[1]][5],tplt[[2]][5],ncol=2))
dev.off()
```
