...
```{r}
lc3 <- readRDS("processed-data/14_apps_prep/01-Samui_TissSect_SPE_RotsMirrors_logcounts_lowres.RDS")
...
## choose one sample per donor to plot
tmpcd <- unique(as.data.table(colData(lc3))[,.(brnum,sample_id)])[sample_id %in% plotg$variable]
set.seed(42)
tmpcd <- tmpcd[,.(sample_id=sample(sample_id,1)),by=brnum]
lc3 <- lc3[,lc3$sample_id %in% tmpcd$sample_id]
...
lapply(rownames(lc3),FUN=function(g){
    ...
    plts <- bpmapply(s=samps,n=names(samps),SIMPLIFY=FALSE,BPPARAM=MulticoreParam(8),FUN=function(s,n){
        ...
        p <- make_escheR(s)
        p <- p |> add_ground("label",stroke=0.6,point_size=1.3)
        p <- p |> add_fill("log counts",size=1.3,point_size = 1.3)
