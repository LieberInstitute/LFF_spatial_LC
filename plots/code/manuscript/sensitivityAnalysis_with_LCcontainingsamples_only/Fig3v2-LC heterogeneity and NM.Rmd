...
3E UPPER PORTION: LC NMF factors 3-8 (3-7 are LC, 8 is astrocytic): enrichr and projection of SPE NMF into WebDiv snseq
...
enrichrpltdat <- enrichrpltdat[LCnmf_filt_factor%in%paste0("LCnmf",c(3,5:8)),]
...
keepterm <- unique(enrichrpltdat[P.value<0.05&LCnmf_filt_factor%in%paste0("LCnmf",c(3,5:8)),.(Enrichr_database,Term)])
...
keepterm[Term %in% c("PERTURB 2483 MUT ESR1 SRP254635 UP","PERTURB 2487 MUT ESR1 SRP254635 UP","ESR1 KD human GSE26298 sample 479","ESR1 Up"),pltterm:="*ESR1* KD/KO Upreg."]
keepterm[Term=="ESR1 knockdown 96h HCC515"&Enrichr_database=="L1000_Kinase_and_GPCR_Perturbations_down",pltterm:="*ESR1* KD/KO Downreg."]
keepterm[Term=="ESR1 knockdown 96h HCC515"&Enrichr_database=="L1000_Kinase_and_GPCR_Perturbations_up",pltterm:="*ESR1* KD/KO Upreg."]
...
## for some reason lines keep getting dropped from running all these calls in series, so make sure we end up with 13 terms (there are multiple for some ESR1 sets)
stopifnot(length(unique(keepterm$pltterm))==13)
...
## reorder pltterm to make more sense. get rid of xbp1s OE associated genes, not FDR sig in any test
pltdat <- pltdat[pltterm!="Ms *Xbp1s* OE Diff. Expr."]
...
pltdat[,LCnmf_filt_factor:=factor(LCnmf_filt_factor,levels=paste0("LCnmf",c(3,5,6,7,8)))]


pdf("plots/manuscript/Fig3/3Dupper-Lowexpfilt_LCnmf_FactorSpecGene_Enrichr_nmfs35678_maxAdjP0.2.pdf",height=4,width=4.35)
...
```{r}
webdivnmf <- readRDS("processed-data/10_subclustering/10e-speLCnmfPrefilt_projectToWeberDivechaSnseq.RDS")
webdivnmf <- webdivnmf[nmf %in% paste0("LCspeFiltNMF_",c(3,5:8))]
webdivnmf[,nmf:=gsub(nmf,pattern="LCspeFiltNMF_(.*)$",replacement=paste0("LCnmf","\\1"))]
webdivnmf[,nmf:=factor(nmf,levels=paste0("LCnmf",c(3,5:8)))]

## we'll remove the x axis labels in illustrator so we know for sure they're lined up when we start.
pdf("plots/manuscript/Fig3/3D LOWER-speLCnmfPrefilt_allfactors_projectToWeberDivechaSnseq.pdf",height=3,width=3.5)
ggplot(webdivnmf, aes(x=nmf, y=webdivClus, size=prop, color=scaled.avg))+
  geom_count()+
  theme_bw()+
  scale_size(range=c(0,3))+
  scale_x_discrete(position="top") +
  scale_color_viridis_c(option="F")+
  labs(x="Spatial LC NMF Factor",y="Weber Divecha Cluster")+
  theme(axis.text.x=element_text(size=7,angle=90,colour = "#000000"),
          axis.text.y=element_markdown(size=7,colour = "#000000"),
          axis.title.x=element_text(size=8,colour = "#000000"),
          axis.title.y=element_text(size=8,colour = "#000000", margin=margin(0,0,0,-0.02,unit="in")),
          legend.title=element_text(size=8,hjust=0.5,colour = "#000000"),
