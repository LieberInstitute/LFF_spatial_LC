...
## get top 20 genes, by logFC, per cluster
mkg <- unique(globalmk[ , .SD[order(-logFC) , ][1:20] , by = "cluster"][,.(ensembl,gene,cluster)])
## pick the ones readers will know
mkg <- mkg[gene %in% c("ERMN", "PKP4", "MBP", "MOBP", "KLK6", "DBH", "TH", "SLC6A2", "PHOX2A", "ADRA2A", "SOX9", "APOE", "GJA1", "ATP1A2", "CAPS", "PLTP", "EFEMP1", "SLC6A4", "ACHE", "HTR2C", "TAC1", "ADCYAP1", "SST", "CCK", "HBA1", "HBA2", "HBB", "ACTA2", "TAGLN")]
...
for (i in c(1:length(names(pal2)))){
    cllookup <- anno[plot_anno==names(pal2)[i],anno]
    if(i==1){
        mkg2 <- mkg[cluster==cllookup,ensembl]
    } else {
        mkg2 <- unique(c(mkg2,mkg[cluster==cllookup,ensembl]))
    }
}
mkg<-mkg2
