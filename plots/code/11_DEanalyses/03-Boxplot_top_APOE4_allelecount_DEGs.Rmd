...
dgehittabs <- list()
dgehittabs[[1]] <- fread("processed-data/11_DEanalyses/03b-E4alleleCount_DE_FDRunder0.2_byGene_byNmodelsOf4max_byCluster_10spotmin.txt")
dgehittabs[[2]] <- fread("processed-data/11_DEanalyses/03c-E4alleleCount_DE_unadjP.05_byGene_byNmodelsOf4max_byCluster_10spotmin.txt")
dgehittabs[[3]] <- fread("processed-data/11_DEanalyses/03d-Sex_DE_E4alleleCountCovar_FDRunder0.2_byGene_byNmodelsOf4max_byCluster_10spotmin.txt")
dgehittabs[[4]] <- fread("processed-data/11_DEanalyses/03e-Sex_DE_E4alleleCountCovar_unadjP.05_byGene_byNmodelsOf4max_byCluster_10spotmin.txt")

names(dgehittabs) <- c("E4count_FDRpt2","E4count_nom","sex_FDRpt2","sex_nom")
...
# 55 gene-cluster pairs that were FDR<0.2 in 3+ models for e4 de; 30 that were FDR<0.2 in all 4 models
...
pdf("plots/11_DEanalyses/03_E4copycount_DE_boxplots/E4alleleCount_DE_3plusmodels_FDR0.2_boxplots.pdf",height=length(apoplot)*4,width=9)
...
### do the same for sex, except limit to 4/4 models and LFC 0.75+.
# load the DE result object to get the log2FC values
```{r}
deres <- readRDS("processed-data/11_DEanalyses/03a-E4andSexDE_with_e4alleleCountModel_toptabs_testsWithandWithout_RINandorAncestry_10spotmin.RDS")
...
# 553 genes-cluster pairs, and we don't have to drop any clusters.
...
for (i in c(1:length(sexplots))){
    pdf(paste0("plots/11_DEanalyses/03_E4copycount_DE_boxplots/",names(sexplots)[i],"_Sex_DE_FDR0.2_boxplots.pdf"),height=length(sexplots[[i]])*4,width=9)
