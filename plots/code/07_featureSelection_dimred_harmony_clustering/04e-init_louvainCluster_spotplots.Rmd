...
SNNlouvlist <- readRDS("processed-data/07")
...
lapply(colnames(SNNlouvs),FUN=function(clus){
    ...
    colLabels(lc3) <- factor(SNNlouvs[, colidx],levels=paste0("X",c(1:nclusts)))
...
snn10louv1 <- as.data.table(cbind(colnames(lc3),snn10louv1))
snn10louv1 <- merge.data.table(snn10louv1,annot,by.x="snn10louv1",by.y="clus")
snn10louv1 <- DataFrame(snn10louv1,row.names=snn10louv1$V1)[colnames(lc3),]

colLabels(lc3) <- snn10louv1$label

lc3sub <- lc3[,lc3$label %in% c("Inh.SST.CCK.HTR2C","LC.1","5HT.1","Exc.17A6.RBP4.TAC1.HTR2C","Unsure.2.PCDHGC5.AVP")]
lc3sub$label <- factor(lc3sub$label,levels=c("Inh.SST.CCK.HTR2C","LC.1","5HT.1","Exc.17A6.RBP4.TAC1.HTR2C","Unsure.2.PCDHGC5.AVP"))

### write plot
nclusts <- length(unique(lc3sub$label))
pal <- palette36.colors(n = nclusts+2)[c(3:(nclusts+2))]
# next line is needed for consistency in color-cluster assignments between libdspatial and the generated legend below
names(pal) <- levels(lc3sub$label)

## create a unified legend spanning all possible cluster values (regardless of presence in a given sample)
leg.tmp <- ggplot(as.data.table(colData(lc3sub))[,.(label,array_row,array_col)],aes(x=array_row,y=array_col,col=label))+geom_point()+
...
dev.off()
### make spotplots
s <- unique(lc3sub$sample_id)

plt <- lapply(s,FUN=function(s){
   cursub <- lc3sub[,lc3sub$sample_id==s]
